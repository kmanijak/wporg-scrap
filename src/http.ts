...
export function createHttpClient({ userAgent }: { userAgent: string }): HttpClient {
  return {
      ...
      while (true) {
        ...
        try {
          ...
          if (res.status === 429) {
            ...
            const waitMs =
              (Number.isFinite(retryAfterSec) ? retryAfterSec : 30) * 1000;
            console.error(
          ...
          if (res.status >= 500) {
            if (serverErrorRetries >= MAX_5XX_RETRIES) {
              throw new HttpBailError(
                `${res.status} exceeded retries at ${url}`,
                res.status,
              );
            }
